  ...
  try {
    ...
    const response = await axios.get(
      `https://nominatim.openstreetmap.org/search`,
      {
        ...
        headers: {
          'User-Agent': 'StudyDen-App/1.0 (studyden@example.com)'
        },
        timeout: 5000
      }
    );
  ...
  try {
    ...
    if (recentEvents >= 3) {
      return res.status(429).json({ 
        message: "You're creating too many events. Please wait before creating more." 
      });
    }
    ...
    for (const existingEvent of existingEvents) {
      ...
      existingEventDate.setHours(existingHours, existingMinutes, 0, 0);

      const timeDiff = Math.abs(newEventDate - existingEventDate) / (1000 * 60 * 60);
      
      if (timeDiff < 3) {
        return res.status(400).json({ 
          message: "You must have at least 3 hours gap between your events" 
        });
      }
    }
...
exports.getEvents = async (req, res, next) => {
  try {
    ...
    const events = await Event.find(filter)
      .populate("host", "name email points badges profilePhoto")
      .sort({ date: 1 })
      .skip(skip)
...
exports.joinEvent = async (req, res, next) => {
  try {
    ...
    if (recentJoins >= 5) {
      return res.status(429).json({ 
        message: "You're joining too many events. Please wait before joining more." 
      });
    }
    ...
    user.joinedEvents = [...(user.joinedEvents || []), event._id];
    await user.save();
    ...
    res.json({ 
      ...
      event: {
        ...event.toObject(),
        isAttending: true,
        spotsFilled: event.attendees.length
      }
    });
  } catch (err) {
    next(err);
...
exports.leaveEvent = async (req, res, next) => {
  try {
    ...
    if (recentLeaves >= 5) {
      return res.status(429).json({ 
        message: "You're leaving too many events. Please wait before leaving more." 
      });
    }
    ...
    const user = await User.findById(req.user.id);
    const joinedEventIndex = user.joinedEvents?.findIndex(
      eId => eId.toString() === event._id.toString()
    );

    if (joinedEventIndex !== -1 && joinedEventIndex !== undefined) {
      user.joinedEvents.splice(joinedEventIndex, 1);
      await user.save();
    }
...
exports.deleteEvent = async (req, res, next) => {
  try {
    ...
    if (recentCancellations >= 2) {
      return res.status(429).json({ 
        message: "You're cancelling too many events. Please wait before cancelling more." 
      });
    }
    ...
    if (hoursUntilEvent < 24 && req.user.role !== "admin") {
      return res.status(400).json({ 
        message: "Cannot delete event within 24 hours of start time" 
      });
    }
    ...
    res.json({ 
      ...
      penalty: hoursUntilEvent < 48 ? "Points deducted for cancellation" : "No penalty"
    });
  } catch (err) {
    next(err);
  }
...
exports.completeEvent = async (req, res, next) => {
  try {
    ...
    if (hostDuration < 30) {
      return res.status(400).json({ 
        message: `Host must be at the event location for at least 30 minutes. Current duration: ${Math.floor(hostDuration)} minutes` 
      });
    }
        ...
        if (isWithinRadius) {
          ...
          if (continuousMinutes >= 1 && !timer.present) {
            ...
            if (!event.attendeesPresent.includes(req.user.id)) {
              event.attendeesPresent.push(req.user.id);
            }
          }
...
exports.submitEndorsement = async (req, res, next) => {
  try {
    ...
    const endorsementPercentage = totalPresent > 0 
      ? (totalEndorsements / totalPresent) * 100 
      : 0;
    ...
    res.json({
      ...
      message: event.isSuccessful 
        ? "Event successful! Host earned bonus points." 
