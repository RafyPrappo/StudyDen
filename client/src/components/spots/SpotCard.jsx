...
import {
  MapPin, Wifi, Zap, Snowflake, VolumeX, Coffee, Car, Bath, Users,
  Trophy, Medal, Trash2, Loader2, Star, Heart,
  CheckCircle, AlertTriangle, Building2,
} from "lucide-react";

const amenityIcons = {
  WiFi: Wifi, "Charging Points": Zap, AC: Snowflake, "Quiet Zone": VolumeX,
  Snacks: Coffee, Parking: Car, Washroom: Bath, "Group Seating": Users,
};
...
export default function SpotCard({ spot, onUpdate, favouriteIds = [] }) {
  ...
  const handleDelete = async () => {
    if (!window.confirm("Are you sure you want to delete this study spot?")) return;
    setDeleting(true);
    try {
  ...
  return (
    <>
      <Card className="p-5 hover:shadow-lg transition-all duration-200 border border-gray-100 hover:border-blue-200 min-h-[420px] flex flex-col">
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" onClick={handleToggleFavourite} disabled={favouriteLoading}
              className={`px-3 py-2 border ${isFavourite ? "border-pink-300 text-pink-600" : "border-gray-200 hover:border-pink-300 hover:text-pink-600"}`}>
              {favouriteLoading ? <Loader2 size={16} className="animate-spin" /> : <Heart size={16} fill={isFavourite ? "currentColor" : "none"} />}
            </Button>
            {!isOwner && (
              <Button variant="ghost" onClick={() => setShowReportModal(true)} className="px-3 py-2 border border-gray-200 hover:border-yellow-300 hover:text-yellow-600">
                Report
              </Button>
            )}
            {isOwner && (
              <Button variant="ghost" onClick={handleDelete} disabled={deleting} className="px-3 py-2 border border-gray-200 hover:border-red-300 hover:text-red-600">
        ...
        <div className="flex-shrink-0">
          <Link to={`/spots/${spot._id}`} className="block">
            <h3 className="text-lg font-semibold text-gray-900 hover:text-blue-600 transition-colors line-clamp-2 min-h-[3.5rem]">{spot.title}</h3>
          </Link>
        </div>
        ...
        <div className="mb-4 flex items-center gap-2 text-sm flex-wrap">
          ...
          <span className="text-gray-500 text-xs">({totalReviews} review{totalReviews === 1 ? "" : "s"})</span>
        ...
        <div className="mb-4 flex items-center gap-2 text-sm">
          <div className="inline-flex items-center gap-1 rounded-full bg-blue-50 text-blue-700 px-3 py-1.5 border border-blue-100">
            <Users size={14} />
            <span className="font-medium">Crowd Status: {spot.crowdStatus || "N/A"}</span>
          </div>
        </div>

        <p className="text-sm text-gray-600 line-clamp-4 min-h-[6rem]">{spot.description || "No description provided."}</p>
          ...
          {spot.amenities?.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {spot.amenities.slice(0, 4).map((amenity) => {
                ...
                return (
                  <span key={amenity} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs bg-gray-100 text-gray-700">
                    {Icon ? <Icon size={12} /> : null}{amenity}
                  </span>
                );
              })}
          ...
          <div className="flex items-center gap-3 p-2 -mx-2 rounded-lg">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 overflow-hidden flex-shrink-0">
              {spot.postedBy?.profilePhoto ? (
                <img src={`${import.meta.env.VITE_API_BASE_URL}${spot.postedBy.profilePhoto}`} alt={spot.postedBy?.name} className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-gray-900 truncate">{spot.postedBy?.name || "Unknown"}</p>
              <div className="flex items-center gap-2 text-xs text-gray-500">
                <span className="flex items-center gap-1 flex-shrink-0"><Trophy size={12} />{spot.postedBy?.points || 0} pts</span>
      ...
      {showReportModal && <ReportSpotModal spotId={spot._id} onClose={() => setShowReportModal(false)} />}
    </>
  );
}